...
import io
import cv2
from fastapi import FastAPI, Form, UploadFile, File, HTTPException, status, Depends
...
@app.post("/predict_and_annotate", response_class=Response, responses={200: {"content": {"image/jpeg": {}}}})
def predict_and_annotate_route(
    file: UploadFile = File(...),
    predictor: CarBrandPredictor = Depends(get_predictor)
):
    annotated_image_bytes = predict_and_annotate(predictor, file)
